...
class AR:
    ...
    def fit(self, y, p = 2):
        ...
        residuals = yt - X @ self.c
        self.var = np.var(residuals)
...
def stationary(ar):
    fL = np.hstack(([1], -ar.c))
    # rts = np.roots(np.flip(fL))
    fL /= fL[-1]
    fL = fL[:-1]
    rts = roots(fL)
    return np.all(np.abs(rts) > 1)
...
if __name__ == '__main__':
    ...
    ar = AR()
    ar.greedy(y, 5)
    print(f'Greedy({len(ar.c)}) stationary? ', stationary(ar))

    # (2) Lasso Regularization

    ar = AR()
    ar.lasso(y, 5)
    print(f'Lasso({len(ar.c)}) stationary? ', stationary(ar))

    # (3) Basic Model

    ar = AR()
    ar.fit(y, 3)
    print(f'Basic({len(ar.c)}) stationary? ', stationary(ar))