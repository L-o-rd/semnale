...
if __name__ == '__main__':
    ...
    Y_high = Y.copy()
    Y_high[:halfy - ifrqcuty, :] = 0 
    Y_high[halfy + ifrqcuty:, :] = 0 
    Y_high[:, :halfx - ifrqcutx] = 0 
    Y_high[:, halfx + ifrqcutx:] = 0
    Y_high = np.real(np.fft.ifft2(Y_high))

    print('SNR inainte:', ssnr(X, X_cutoff))
    noise = X_cutoff - X
    print('SNR dupa:', ssnr(X, X_cutoff - noise))
    print('SNR dupa:', ssnr(X, Y_high + X_cutoff))
    SNR = 31
    YS = isnr(SNR, X)
    fig, axs = plt.subplots(2, 2, figsize = (10, 6))
    axs[0, 0].imshow(X, cmap = 'gray')
    axs[0, 1].imshow(freq_db)
    axs[1, 0].imshow(Y_high + X_cutoff, cmap = plt.cm.gray)
    axs[1, 1].imshow(X_cutoff, cmap=plt.cm.gray)
    plt.savefig('./labs/07/03.pdf')
    plt.show()